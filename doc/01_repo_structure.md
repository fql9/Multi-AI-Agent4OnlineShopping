# 01｜仓库结构（落地版：Python Agent + TypeScript API + Monorepo）

> 本文档给出一个**可直接照抄创建**的目录结构，围绕"Contract First + Python Agent + TypeScript API"组织。

---

## 核心原则

1. **Python 做 Agent 编排**：LangGraph + Pydantic + FastAPI，LLM 生态更成熟
2. **TypeScript 做 Tool Gateway / MCP**：强类型 API，前后端一致
3. **Contract First**：所有工具先定义 schema，Python/TypeScript 各自生成 typesafe client
4. **Monorepo**：便于 Contract 共享与一致性校验

---

## 目录树（当前实现）

```
/
├── README.md                         # 项目主文档
├── CONTRIBUTING.md                   # 贡献指南
├── LICENSE                          # 许可证 (GPLv3)
├── SECURITY.md                      # 安全政策
│
├── doc/                             # 🔴 设计与规范文档
│   ├── README.md                    # 文档索引
│   ├── 00_overview.md              # 项目概览
│   ├── 01_repo_structure.md        # 仓库结构（本文档）
│   ├── 02_tech_stack.md            # 技术栈
│   ├── ...
│   ├── 17_progress.md              # 开发进度
│   └── 18_deployment.md            # 部署指南
│
├── contracts/                       # 🔴 核心：Contract First
│   ├── json-schema/                 # 共享 JSON Schema
│   │   └── models/
│   │       ├── envelope.schema.json # 统一请求/响应 Envelope
│   │       └── mission.schema.json  # Mission 数据模型
│   └── error-codes.yaml             # 统一错误码定义
│
├── packages/                        # 共享包（TypeScript）
│   └── common/                      # 日志、错误码、Envelope、trace、重试
│       ├── src/
│       │   ├── db.ts               # 数据库连接池
│       │   ├── envelope.ts         # Envelope 工具
│       │   ├── errors.ts           # 错误处理
│       │   ├── logger.ts           # 结构化日志
│       │   ├── retry.ts            # 🆕 重试/熔断/超时
│       │   ├── trace.ts            # OpenTelemetry
│       │   └── index.ts
│       ├── package.json
│       └── tsconfig.json
│
├── agents/                          # 🐍 Python：Agent 编排层
│   ├── pyproject.toml               # Python 项目配置
│   ├── Dockerfile                   # Docker 镜像
│   ├── README.md
│   ├── src/
│   │   ├── __init__.py
│   │   ├── config.py               # Pydantic Settings 配置
│   │   ├── server.py               # 🆕 FastAPI HTTP Server
│   │   ├── main.py                 # CLI 入口
│   │   ├── cli.py                  # 命令行工具
│   │   ├── graph/                  # LangGraph 状态机
│   │   │   ├── __init__.py
│   │   │   ├── builder.py          # 状态机构建
│   │   │   └── state.py            # AgentState 定义
│   │   ├── orchestrator/           # Orchestrator Agent
│   │   │   ├── __init__.py
│   │   │   └── session.py          # 会话管理
│   │   ├── intent/                 # Intent Agent
│   │   │   ├── __init__.py
│   │   │   └── node.py
│   │   ├── candidate/              # Candidate Agent
│   │   │   ├── __init__.py
│   │   │   └── node.py
│   │   ├── verifier/               # Verifier Agent
│   │   │   ├── __init__.py
│   │   │   └── node.py
│   │   ├── compliance/             # Compliance Agent
│   │   │   ├── __init__.py
│   │   │   └── node.py
│   │   ├── execution/              # Execution/Payment Agent
│   │   │   ├── __init__.py
│   │   │   ├── plan_node.py
│   │   │   ├── execution_node.py
│   │   │   └── payment_node.py
│   │   ├── tools/                  # LangChain Tools 封装
│   │   │   ├── __init__.py
│   │   │   ├── base.py             # 基础工具
│   │   │   ├── catalog.py          # Catalog 工具
│   │   │   ├── pricing.py          # Pricing 工具
│   │   │   ├── shipping.py         # Shipping 工具
│   │   │   ├── compliance.py       # Compliance 工具
│   │   │   ├── checkout.py         # Checkout 工具
│   │   │   ├── evidence.py         # Evidence 工具
│   │   │   └── knowledge.py        # 🆕 Knowledge/RAG 工具
│   │   ├── models/                 # Pydantic 数据模型
│   │   │   ├── __init__.py
│   │   │   ├── mission.py
│   │   │   ├── draft_order.py
│   │   │   └── evidence.py
│   │   └── llm/                    # LLM 客户端
│   │       ├── __init__.py
│   │       ├── client.py           # OpenAI API 封装
│   │       ├── prompts.py          # Agent 提示词
│   │       └── schemas.py          # 输出 Schema
│   └── tests/
│       ├── __init__.py
│       ├── test_agent_flow.py
│       └── test_models.py
│
├── apps/                            # 🟦 TypeScript：API / MCP / 前端
│   ├── tool-gateway/                # 统一 Tool Gateway
│   │   ├── Dockerfile
│   │   ├── package.json
│   │   ├── tsconfig.json
│   │   └── src/
│   │       ├── index.ts            # Fastify 入口
│   │       ├── config.ts           # 配置
│   │       ├── middleware/
│   │       │   ├── audit.ts        # 审计日志
│   │       │   ├── envelope.ts     # Envelope 校验
│   │       │   └── idempotency.ts  # 幂等性
│   │       ├── routes/
│   │       │   ├── index.ts
│   │       │   ├── catalog.ts
│   │       │   ├── pricing.ts
│   │       │   ├── shipping.ts
│   │       │   ├── compliance.ts
│   │       │   ├── checkout.ts
│   │       │   ├── evidence.ts
│   │       │   └── knowledge.ts    # 🆕 Knowledge 路由
│   │       └── services/
│   │           └── xoobay.ts       # 🆕 XOOBAY API 客户端
│   │
│   ├── mcp-servers/                 # MCP Servers (SSE 模式)
│   │   ├── core-mcp/                # 核心工具服务
│   │   │   ├── Dockerfile
│   │   │   ├── package.json
│   │   │   ├── tsconfig.json
│   │   │   └── src/
│   │   │       ├── index.ts        # SSE Server 入口
│   │   │       ├── db.ts           # 数据库连接
│   │   │       ├── catalog/
│   │   │       │   └── index.ts
│   │   │       ├── pricing/
│   │   │       │   └── index.ts
│   │   │       ├── shipping/
│   │   │       │   └── index.ts
│   │   │       ├── compliance/
│   │   │       │   └── index.ts
│   │   │       ├── tax/
│   │   │       │   └── index.ts
│   │   │       └── knowledge/      # 🆕 RAG 工具
│   │   │           └── index.ts
│   │   │
│   │   └── checkout-mcp/            # 高敏感：结算服务
│   │       ├── Dockerfile
│   │       ├── package.json
│   │       ├── tsconfig.json
│   │       └── src/
│   │           ├── index.ts        # SSE Server 入口
│   │           ├── cart/
│   │           │   └── index.ts
│   │           ├── checkout/
│   │           │   └── index.ts
│   │           └── evidence/
│   │               └── index.ts
│   │
│   └── web-app/                     # 用户端（Next.js）
│       ├── Dockerfile
│       ├── package.json
│       ├── tsconfig.json
│       ├── next.config.js
│       ├── tailwind.config.ts
│       ├── postcss.config.js
│       └── src/
│           ├── app/
│           │   ├── layout.tsx
│           │   ├── page.tsx
│           │   └── globals.css
│           ├── components/
│           │   └── ui/             # shadcn/ui 组件
│           ├── lib/
│           │   └── utils.ts
│           └── store/
│               └── shopping.ts     # Zustand 状态管理
│
├── data/                            # 数据管道（预留：真实数据导入/ETL/评估）
│
├── infra/                           # 基础设施
│   └── docker/
│       ├── docker-compose.yml      # 基础开发环境
│       ├── init-db.sql             # 数据库初始化
│       └── migrations/
│           ├── 001_update_schema.sql
│           └── 002_enhance_rag.sql # 🆕 RAG 增强
│
├── scripts/                         # 开发脚本
│   ├── generate-sdk.sh
│   ├── import-sample-data.py
│   ├── sync-xoobay-products.ts     # 🆕 XOOBAY 同步
│   └── test-xoobay-api.ts          # 🆕 API 测试
│
├── tools/                           # 辅助工具
│   └── mcp-gh-ci-jobs/             # GitHub CI MCP 工具
│       └── README.md
│
├── .github/
│   └── workflows/
│       └── ci.yml                   # CI/CD 配置
│
├── .env.example                     # 🆕 环境变量模板
├── docker-compose.full.yml          # 🆕 完整 Docker Compose
├── package.json                     # Monorepo 根配置
├── pnpm-workspace.yaml
├── pnpm-lock.yaml
├── tsconfig.base.json
└── turbo.json                       # Turborepo 配置
```

---

## 组件职责边界（强制）

| 组件 | 职责 | 禁止 |
|------|------|------|
| `contracts/` | 定义工具 schema、错误码、数据模型 | 不含业务逻辑 |
| `agents/` | Agent 编排、LLM 调用、状态机、任务拆解 | 不直接访问数据库；强事实必须来自工具 |
| `agents/src/server.py` | HTTP API 暴露 Agent 功能 | 不做业务决策 |
| `tool-gateway/` | Envelope、鉴权、幂等、限流、审计、trace | 不做业务决策 |
| `mcp-servers/` | 领域服务编排，输出强事实与 evidence | 不直接暴露给前端 |
| `data/pipelines/` | 离线/准实时构建 AROC/KG/索引 | 不绕开证据与版本管理 |
| `web-app/` | 用户交互、展示、支付确认 | 不直接调用 Agent |

---

## Docker 服务拓扑

```
┌─────────────────────────────────────────────────────────────────────┐
│                       用户层                                         │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │                    web-app (Next.js)                        │   │
│  │                       :3001                                  │   │
│  └─────────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────────┐
│                      Agent 编排层                                    │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │              agent (Python + LangGraph + FastAPI)           │   │
│  │                       :8000                                  │   │
│  └─────────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────────┐
│                      工具网关层                                      │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │                tool-gateway (Fastify)                       │   │
│  │                       :3000                                  │   │
│  └─────────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────┘
                                │
                ┌───────────────┴───────────────┐
                ▼                               ▼
┌───────────────────────────┐   ┌───────────────────────────┐
│      core-mcp (SSE)       │   │    checkout-mcp (SSE)     │
│          :3010            │   │          :3011            │
└───────────────────────────┘   └───────────────────────────┘
                │                               │
                └───────────────┬───────────────┘
                                ▼
┌─────────────────────────────────────────────────────────────────────┐
│                        数据层                                        │
│  ┌─────────────────────────┐  ┌─────────────────────────┐          │
│  │   postgres (pgvector)   │  │         redis           │          │
│  │         :5433           │  │         :6379           │          │
│  └─────────────────────────┘  └─────────────────────────┘          │
└─────────────────────────────────────────────────────────────────────┘
```

---

## MVP 最小启动

MVP 只需启动以下组件：

```bash
# 一键启动所有服务
docker compose -f docker-compose.full.yml up -d

# 验证服务状态
docker compose -f docker-compose.full.yml ps
```

服务列表：
- ✅ postgres (5433) - 数据库
- ✅ redis (6379) - 缓存
- ✅ tool-gateway (3000) - API 网关
- ✅ core-mcp (3010) - 核心工具
- ✅ checkout-mcp (3011) - 结算工具
- ✅ web-app (3001) - 前端
- ✅ agent (8000) - Python Agent

---

## Monorepo 工具推荐

| 工具 | 用途 |
|------|------|
| **pnpm** | TypeScript 包管理（workspace） |
| **Turborepo** | 构建缓存与任务编排 |
| **uv** 或 **poetry** | Python 依赖管理 |
| **Docker Compose** | 本地开发与部署 |

---

## Contract First 工作流

```
1. 在 contracts/ 定义 OpenAPI / JSON Schema
       ↓
2. 运行 scripts/generate-sdk.sh
       ↓
3. 自动生成：
   - packages/contracts-ts/（TypeScript 类型）
   - agents/src/models/（Pydantic 模型）
       ↓
4. MCP Server / Agent 使用生成的类型开发
       ↓
5. CI 运行 contract-check：确保实现与 schema 一致
```

---

## 环境配置

项目使用 `.env.example` 作为环境变量模板：

```bash
# 复制模板
cp .env.example .env

# 编辑配置
nano .env
```

关键配置项：
- `OPENAI_API_KEY` - OpenAI API 密钥（必需）
- `POSTGRES_PASSWORD` - 数据库密码
- `REDIS_PASSWORD` - Redis 密码
- `XOOBAY_API_KEY` - XOOBAY API 密钥（可选）
